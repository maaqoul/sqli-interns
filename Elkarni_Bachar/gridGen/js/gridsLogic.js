...
const removeItemReference = (item) => {
    ...
    if (referencePanel.lastElementChild === item) {
        ...
        item.remove(); 
        div_Name_IdCounter = elementNum; //=> resets the counter to the deleted item's number
        ...
        for (let i = div_Name_IdCounter-1; i < referencePanel.childNodes.length; i++) {
            referencePanel.childNodes[i].childNodes[0].innerText = "div " + (div_Name_IdCounter); //=> renaming the referencePanel items 
            ...
            showcase.childNodes[i].childNodes[0].innerText = "div " + (div_Name_IdCounter); //=> renaming the showcase grid items
