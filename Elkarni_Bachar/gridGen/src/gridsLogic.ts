let startDiv:number = 0; //=> selected grid item (the starting tile)
let endDiv:number = 0; //=> selected grid item (the ending tile)
let div_Name_IdCounter:number = 1; //=> used to name the items on the showcase (div_1, div_2, div_3...) => the number part
...
const handleTileSelection = () => {
  ...
  let colStart = 0,
    ...
    rowStart = 0,
    rowEnd = 0;
  if (startDiv > endDiv) [startDiv, endDiv] = [endDiv, startDiv]; //=> Swapping the starting and ending positions to avoid negative number when substracting
  ...
  const showCaseItem:HTMLDivElement = document.createElement("div");
  showCaseItem.style.backgroundColor = genRandomColor();
  showCaseItem.style.gridArea = `${rowStart}/${colStart}/${rowEnd + 1}/${
    colEnd + 1
  }`;
  ...
  if(showcase)showcase.appendChild(showCaseItem); //=> Adding the item to the showcase grid
...
const createItemReference = (itemName:string, itemId:string) => {
  ...
  btn.addEventListener("click", (event:Event) => {
    ...
    removeShowcaseTile(btnElement.getAttribute("data-id") as string);
...
const removeShowcaseTile = (id:string) => {
  const showcaseElement:HTMLElement|null = document.getElementById(id);
  showcaseElement!.remove();
};

//=> This removes an entry from within the item reference panel
const removeItemReference = (item: HTMLDivElement) => {
  const elementNum = (item.children[0] as HTMLDivElement).innerText.split("").pop();
  ...
  if (referencePanel!.lastElementChild === item) {
    ...
    item.remove();
    div_Name_IdCounter = parseInt(elementNum as string); //=> resets the counter to the deleted item's number

    //=> Updating the names of all the elements after the removed one
    for (
      let i = div_Name_IdCounter - 1;
      i < referencePanel!.childNodes.length;
      i++
    ) {
      (referencePanel!.childNodes[i].childNodes[0] as HTMLElement).innerText =
        "div " + div_Name_IdCounter; //=> renaming the referencePanel items
      (referencePanel!.childNodes[i].childNodes[1] as HTMLElement).setAttribute(
        "data-id",
        "showcase_" + div_Name_IdCounter
      ); //=> renaming the referencePanel data-id attribute

      (showcase!.childNodes[i].childNodes[0] as HTMLElement).innerText =
        "div " + div_Name_IdCounter; //=> renaming the showcase grid items
      (showcase!.childNodes[i] as HTMLElement).setAttribute(
        "id",
        "showcase_" + div_Name_IdCounter
      ); //=> renaming the showcase grid items
      div_Name_IdCounter++;
    }
  }
};
