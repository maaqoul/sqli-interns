...
const handleInputsChange = (event) => {
    ...
    switch (inputElement.id) {
        ...
        case "colGaps":
            if (grid)
                grid.style.columnGap = `${inputElement.value}px`;
            if (showcase)
                showcase.style.columnGap = `${inputElement.value}px`;
            break;
        case "rowGaps":
            if (grid)
                grid.style.rowGap = `${inputElement.value}px`;
            if (showcase)
                showcase.style.rowGap = `${inputElement.value}px`;
            break;
        default:
            break;
    }
